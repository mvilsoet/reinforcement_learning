...
class TabQPolicy(QPolicy):
    ...
    def discretize(self, obs):
        ...
        """
        upper_bounds = [self.env.observation_space.high[0], 4.8, self.env.observation_space.high[2], math.radians(24)]
        lower_bounds = [self.env.observation_space.low[0], -4.8, self.env.observation_space.low[2], -math.radians(24)]
    ...
    def qvals(self, states):
        ...
        """
        return [self.model[self.discretize(state)] for state in states]
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        prev_qval = self.model[state][action]

        max_next_qval = max(self.model[next_state][0], self.model[next_state][1])
        if done:
            target = reward
        else:
            target = reward+self.gamma*max_next_qval
        self.model[state][action] = prev_qval+self.lr*(target-prev_qval)

        return (target-prev_qval)*(target-prev_qval)
...
if __name__ == '__main__':
    ...
    policy = TabQPolicy(env, buckets=(2, 16, 2, 16), actionsize=actionsize, lr=args.lr, gamma=args.gamma)
